...
async function publish({
  ...
  try {
    ...
    // Clean old gm-landing-page images and dangling images
    ...
    let { stdout: dockerImageTags } = await execWrapper(
      `docker images "deciphernow/gm-landing-page" --format "{{.Repository}}:{{.Tag}}"`
    );
    // Filter out untagged image (deciphernow/gm-landing-page:<none>) if it exists
    if (dockerImageTags.includes("deciphernow/gm-landing-page:<none>")) {
      dockerImageTags = dockerImageTags.replace(
        "deciphernow/gm-landing-page:<none>",
        ""
      );
    }
    if (dockerImageTags.length > 0) {
      await execWrapper(
        `docker rmi --force $(docker images "deciphernow/gm-landing-page" --format "{{.Repository}}:{{.Tag}}")`
      );
    }
    // Remove untagged image (deciphernow/gm-landing-page:<none>) if it exists
    const { stdout: dockerImages } = await execWrapper(
      `docker images "deciphernow/gm-landing-page" --format "{{.ID}}"`
    );
    if (dockerImages.length > 0) {
      await execWrapper(
        `docker rmi --force $(docker images "deciphernow/gm-landing-page" --format "{{.ID}}")`
      );
    }
    ...
    if (major !== null && minor !== null && patch !== null) {
      releaseTags = [
        ...
        ...tags
      ].map(tag => `deciphernow/gm-landing-page:${tag}`);
      ...
      releaseTags = tags.map(tag => `deciphernow/gm-landing-page:${tag}`);
    }
    ...
    let { stdout: imageIDWithEscapeChars } = await execWrapper(
      "docker images deciphernow/gm-landing-page:latest --format '{{.ID}}'"
    );
    ...
    const actualTags = dryRun
      ? [...releaseTags, "deciphernow/gm-landing-page:latest"]
    ...
    await execWrapper(`docker push deciphernow/gm-landing-page`);
